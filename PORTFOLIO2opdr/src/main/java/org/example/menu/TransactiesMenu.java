...
public class TransactiesMenu {
    ...
    private void toonAlleTransacties() throws IOException {
        ...
        List<Inkomsten> inkomstenList = DataStorage.loadInkomsten(budget.getNaam());
        List<Uitgaven> uitgavenList = DataStorage.loadUitgaven(budget.getNaam());
    ...
    private void exporteerNaarCSV() throws IOException {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(csvBestandPath))) {
            ...
            // Schrijf inkomsten
            for (Inkomsten inkomst : DataStorage.loadInkomsten(budget.getNaam())) {
                writer.write("Inkomst," + inkomst.getBron() + "," + inkomst.getBedrag() + "\n");
            }

            // Schrijf uitgaven
            for (Uitgaven uitgave : DataStorage.loadUitgaven(budget.getNaam())) {
